...
void main() {
  ...
  testWidgets(
    'vibe flow: generate, parse, and install an app',
    (tester) async {
      ...
      final model = await llm.ensureModel(onEvent: (event) {
        if (event is LoadProgress) {
          debugPrint('downloading model: ${event.percentage}%');
        } else if (event is LoadError) {
          debugPrint('model load error: ${event.message}');
        }
      });
      expect(model, isNotNull);

      // A faithful version of the Vibe Studio's system prompt.
      final system = await AppGenerator.buildSystemPrompt(installedApps: const []);
